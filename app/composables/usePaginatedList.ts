...
type UsePaginatedListOptions<T, R> = {
  ...
  store: ListStore<T>
  mapResponse?: (res: R) => { data: T[]; pagination: Pagination }
  filters?: Record<string, any> | Ref<Record<string, any>>
}
...
export function usePaginatedList<T, R>({
  ...
  const asyncKey = computed(
    () =>
      `${key}-${route.fullPath}-${page.value}-${pageSize.value}-${debouncedSearch.value}-${JSON.stringify(resolvedFilters.value)}`
  )
  ...
  watch([page, pageSize, debouncedSearch, resolvedFilters], () => refresh())
