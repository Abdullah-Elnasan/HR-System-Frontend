 ...
 */
function convertBooleansToNumbers(obj: any): any {
 ...
 */
function toFormData(obj: any, formData = new FormData(), parentKey = ''): FormData {
 ...
 */
function transformWorkSchedulePayload(payload: any): any {
...
export async function createResource<T>({
  ...
  // تحويل boolean إلى 0/1
  let convertedPayload = payload instanceof FormData
    ? payload
    : convertBooleansToNumbers(payload)

  // تحويل WorkScheduleForm إذا كانت موجودة
  if (
    !(convertedPayload instanceof FormData) &&
    convertedPayload.type &&
    (convertedPayload.uniform_fixed ||
      convertedPayload.uniform_flexible ||
      convertedPayload.custom_fixed_days ||
      convertedPayload.custom_flexible_days)
  ) {
    convertedPayload = transformWorkSchedulePayload(convertedPayload)
  }

  const formData = convertedPayload instanceof FormData
    ? convertedPayload
    : toFormData(convertedPayload)
